...
abstract contract ERC721Attachable is OwnableUpgradeable, ERC721Upgradeable {
    ...
    EnumerableSetUpgradeable.AddressSet private _slaveCollections;

    // Grant the specified address the permission to transfer slave nft for airdrop NFT deposit into the game contract.
    EnumerableSetUpgradeable.AddressSet private _transferApprovals;
    ...
    function transferFrom(
        ...
        if (slaveTokens[tokenId].collection != address(0)) {
            ...
            for (uint256 i = 0; i < _masterTokens[tokenId].length; i++) {
                IERC721Upgradeable(_masterTokens[tokenId][i].collection).transferFrom(
                    from,
                    to,
    ...
    function safeTransferFrom(
        ...
        if (slaveTokens[tokenId].collection != address(0)) {
            ...
            for (uint256 i = 0; i < _masterTokens[tokenId].length; i++) {
                IERC721Upgradeable(_masterTokens[tokenId][i].collection).safeTransferFrom(
                    from,
                    to,
                    _masterTokens[tokenId][i].tokenId,
                    data
