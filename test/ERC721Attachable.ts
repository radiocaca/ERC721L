...
import { ERC721AttachableMock } from "typechain-types";

describe("ERC721Attachable contract", function() {
  let owner: SignerWithAddress;
  let alice: SignerWithAddress;
  let AttachableMaster: ERC721AttachableMock;
  let AttachableSlave: ERC721AttachableMock;
  
  beforeEach(async () => {
    [owner, alice] = await ethers.getSigners();
    
    const contract = await ethers.getContractFactory("ERC721AttachableMock");
    AttachableMaster = await contract.deploy("Master", "M");
    
    AttachableSlave = await contract.deploy("Slave", "S");
  });
